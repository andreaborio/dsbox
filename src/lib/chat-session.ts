...
function restoreMessageStats(message: ChatMessage, restoredAt: number): ChatMessage {
  if (!message.stats) return message.pending ? { ...message, pending: false, interrupted: true } : message;
  const stats: ChatResponseStats = {
    ...message.stats,
    cachedPromptTokens: message.stats.cachedPromptTokens ?? null
  };
  if (message.pending) {
    return { ...message, pending: false, interrupted: true, stats: finalizeStats(stats, restoredAt) };
  }
